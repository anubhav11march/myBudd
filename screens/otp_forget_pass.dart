...
class ForgetVerification extends StatefulWidget {
  ...
  final String email;
//  final bool verify;
  const ForgetVerification(@required this.email);
  @override
  State<ForgetVerification> createState() => _ForgetVerificationState();
}
...
class _ForgetVerificationState extends State<ForgetVerification> {
  ...
  void _onSubmit(BuildContext context, var pin) async {
    var response = await ForgetOtp.verify(pin, widget.email);
      ...
      if (response['success'] != true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(snackBar);
           ...
           const snackBar = SnackBar(
          content: Text("succcccccccccccccccccccess"),
          duration: Duration(milliseconds: 2000),
        );
        ScaffoldMessenger.of(context).showSnackBar(snackBar);
  ...
  void _onResend(BuildContext context) async {
    var response = await AddEmail.verify(widget.email);
